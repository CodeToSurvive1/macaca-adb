...
ADB.prototype.inputByIME = function() {
  var args = Array.prototype.slice.call(arguments);
  var name = args[0];
  var msg = ` am broadcast -a ADB_INPUT_TEXT --es msg ${name}`;

  var promise = new Promise((resolve, reject) => {
      this.shell(msg).then(data => {
          console.log(data);
          resolve(data);
      }).catch(err => {
          reject(`exec ${msg} error with: ${err}`);
      });
  });
  ...
  if (args.length > 1) {
      ...
      promise.then(data => {
          ...
          cb.call(this, `exec ${msg} error with: ${err}`);
      });
  } else {
      return promise;
  }
